...
function zip() {
	...
	var maxLen = 0;
	if (!argsLen) throw new Error("zip requires at least one argument");
	for (var i = 0; i < argsLen; i++) {
		if (!Array.isArray(args[i])) throw new Error("all arguments must be arrays");
		var arrLen = args[i].length;
		if (arrLen > maxLen) maxLen = arrLen;
	}
	var result = [];
	var group = [];
	var p1 = 0;
